...
- [ ] **Orquestación estándar / gateway único.** No implementado. A la fecha de este informe la cuenta tiene ~40 conectores MCP activos simultáneamente en una sola sesión de Claude Code — es el escenario exacto de "orchestration sprawl" que el reporte de Docker marca como el problema #1 (48% de las orgs). No hay un gateway/registro central que decida qué conector se activa para qué tarea; se dependen todos a la vez. Recomendación concreta: revisar periódicamente qué conectores están realmente en uso (vs. configurados "por si acaso") y desactivar los que no se usan — reduce superficie de ataque y la carga cognitiva de tener 40 herramientas disponibles para cada tarea.
