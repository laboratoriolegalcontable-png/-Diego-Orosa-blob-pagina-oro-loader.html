...
- [x] **Orquestación estándar / gateway único — auditado (2026-09-12).** No implementado un gateway, pero se hizo la auditoría inicial: **51 conectores** reales conectados y habilitados simultáneamente (número exacto vía `ListConnectors`, no la estimación de "~40" que se usó originalmente acá) — el escenario "orchestration sprawl" del reporte de Docker (problema #1, 48% de las orgs) confirmado con datos reales, no solo en teoría. Ver `informes/2026-09-12-auditoria-conectores-mcp.md` para el detalle: 5 hallazgos concretos de solapamiento (dos backends Postgres compitiendo — Supabase real + Neon sin uso declarado; 10 plataformas de hosting cuando la arquitectura real es Netlify+Supabase+Make; 5 canales de email simultáneos, uno roto; 5 herramientas de ads/SEO solapadas; 4 conectores sin conexión aparente al negocio). Las bajas de conectores quedan pendientes de que el Doctor las confirme — no es algo para automatizar sin su decisión.
